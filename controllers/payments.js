     ...
     newPayment.save(async (err, newPayment) => {
       ...
       else 
      {
        console.log(newPayment);
         const pi = await SignedPiPDF.findOne({pi_id : piId},{pi_id : 1 , pi_no : 1 , employee : 1 , manager : 1 
        , date : 1 , name : 1 , status : 1 , pi_done_status : 1 , buyer_address : 1 , createdAt : 1 , updatedAt : 1 , payments : 1  ,piId : 1}).sort({createdAt : -1})
...
export const getAllPayments = async (req, res) => {
      try {
        ...
        const payments = await payment.find({},{invoiceNo : 1 , piId : 1 , amount : 1 
          , date : 1 , employee : 1 , customer : 1 , createdAt : 1 , updatedAt : 1 , piId : 1 }).sort({createdAt : -1});
        res.json(payments);
      } catch (error) {
    ...
    export const getPiPayments = async (req, res) => {
        ...
        try {
         console.log(piId);
          const piPayments = await payment.find({piId : piId},{invoiceNo : 1 , piId : 1 , amount : 1 
            , date : 1 , employee : 1 , customer : 1 , createdAt : 1 , updatedAt : 1  }).sort({createdAt : 1});
